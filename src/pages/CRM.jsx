...
function SmsModal({ contact, onClose }) {
  ...
  const send = async () => {
    ...
    if (res.data?.success) {
      ...
      setResult('❌ ' + (res.data?.error || 'Failed to send. Check WhatsApp credentials in Supabase secrets.'))
    }
    setSending(false)
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
        <h2 className="text-lg font-bold mb-1 flex items-center gap-2" style={{ color: '#0A1628' }}>
          <span style={{ color: '#25D366' }}>{WA_ICON}</span> WhatsApp — {contact.name}
        </h2>
        ...
        <div className="space-y-3">
          ...
          <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Write your WhatsApp message…" rows={6}
          ...
          <div className="flex gap-3">
            ...
            <button onClick={send} disabled={sending || !contact.phone}
              ...
              {sending ? 'Sending…' : <>{WA_ICON} Send WhatsApp</>}
            </button>
          </div>
        </div>
      </div>
                ...
                <div className="flex gap-1.5 ml-3 flex-shrink-0">
                  ...
                  {c.phone && (
                    <button
                      ...
                      className="text-xs px-2.5 py-1.5 rounded-lg font-semibold"
                      style={{ background: '#DCFCE7', color: '#25D366' }} className="text-xs px-2.5 py-1.5 rounded-lg font-semibold flex items-center gap-1">
                      {WA_ICON} WhatsApp</button>
                  )}
                  {c.phone && (
                    <a href={`tel:${c.phone}`}
                      className="text-xs px-2.5 py-1.5 rounded-lg font-semibold"
                    ...
                    <td className="py-3 px-4 text-sm text-gray-500">
                      {contact.phone ? (
                        <div className="flex items-center gap-2">
                          ...
                          <button onClick={() => onSms && onSms({ name: `${contact.first_name} ${contact.last_name}`, phone: contact.phone, companyId: contact.company_id, companyName: contact.company?.name })} className="text-xs px-2 py-0.5 rounded-lg flex-shrink-0 flex items-center" style={{ background: '#DCFCE7', color: '#25D366' }}>{WA_ICON}</button>
